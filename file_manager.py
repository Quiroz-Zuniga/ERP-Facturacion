...
class FileManager:
    ...
    def get_data_from_db(self, table_name):
        ...
        if table_name == "Productos":
            return self.db.fetch(
                "SELECT id, nombre, descripcion, precio, stock, proveedor_id, imagen_path FROM Productos"
            )
            ...
            return self.db.fetch(
                "SELECT id, nombre, tipo, porcentaje FROM Descuentos"
            )
        return []
    ...
    def export_data(self, table_name):
        ...
        try:
            with open(file_path, 'w', newline='', encoding='utf-8') as f:
                ...
                headers = {
                    "Productos": ["id", "nombre", "descripcion", "precio", "stock", "proveedor_id", "imagen_path"],
    ...
    def import_products(self, app_reference):
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    if len(row) >= 6:
                        # nombre, desc, precio, stock, prov_id, img_path
                        product_list.append((
                            row[1],  # nombre
                            row[2],  # descripcion
                            float(row[3]),  # precio
                            int(row[4]),  # stock
                            int(row[5]),  # proveedor_id
                            row[6] if len(row) > 6 else ""  # imagen_path
                        ))
    ...
    def show_import_preview(self, product_list, app_reference):
        ...
        # Cargar datos
        for i, (nombre, desc, precio, stock, prov_id, img_path) in enumerate(product_list):
            tree.insert(
                "", "end", 
                iid=i, 
        ...
        def edit_selected():
            ...
            fields = ["Nombre", "Descripción", "Precio", "Stock", "Proveedor ID", "Imagen"]
            
            for i, field in enumerate(fields):
                ttk.Label(edit_win, text=f"{field}:").grid(row=i, column=0, padx=10, pady=5, sticky="w")
                ttk.Entry(edit_win, textvariable=vars_list[i], width=30).grid(row=i, column=1, padx=10, pady=5)
        ...
        def confirm_import():
            ...
            query = """INSERT INTO Productos 
                       (nombre, descripcion, precio, stock, proveedor_id, imagen_path) 
                       VALUES (?, ?, ?, ?, ?, ?)"""
