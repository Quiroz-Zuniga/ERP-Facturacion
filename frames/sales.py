...
class SalesFrame(ttk.Frame):
    ...
    def open_product_search(self):
        ...
        search_entry.bind('<KeyRelease>', lambda e: self.filter_search(tree, search_var.get()))
        ...
        tree.bind('<<TreeviewSelect>>', lambda e: self.show_product_detail(tree, detail_frame))
        
        # Panel de detalle
        detail_frame = ttk.LabelFrame(search_win, text="Detalle del Producto", padding=10)
        ...
        ttk.Spinbox(qty_frame, from_=1, to=100, textvariable=qty_var, width=10).pack(side="left", padx=5)
    ...
    def remove_from_cart(self):
        ...
        prod_id = self.cart_tree.item(selected, 'values')[0]
