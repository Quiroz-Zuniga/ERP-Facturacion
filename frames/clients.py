...
class ClientsFrame(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        # Título principal
        title_label = ttk.Label(self, text="GESTIÓN DE CLIENTES", font=("Arial", 16, "bold"))
        ...
        left_frame = ttk.LabelFrame(main_frame, text="Información del Cliente", padding=15)
        ...
        # Nombre
        ttk.Label(left_frame, text="Nombre *:").grid(row=0, column=0, sticky="w", pady=5)
        self.nombre_entry = ttk.Entry(left_frame, textvariable=self.nombre_var, width=30)
        self.nombre_entry.grid(row=0, column=1, columnspan=2, sticky="ew", pady=5)

        # Apellido
        ttk.Label(left_frame, text="Apellido *:").grid(row=1, column=0, sticky="w", pady=5)
        self.apellido_entry = ttk.Entry(left_frame, textvariable=self.apellido_var, width=30)
        self.apellido_entry.grid(row=1, column=1, columnspan=2, sticky="ew", pady=5)

        # DNI/Identidad
        ttk.Label(left_frame, text="DNI/Identidad:").grid(row=2, column=0, sticky="w", pady=5)
        ...
        self.telefono_entry = ttk.Entry(left_frame, textvariable=self.telefono_var, width=30)
        ...
        # Dirección
        ttk.Label(left_frame, text="Dirección:").grid(row=5, column=0, sticky="w", pady=5)
        ...
        # Estado activo
        self.activo_check = ttk.Checkbutton(left_frame, text="Cliente Activo", variable=self.activo_var)
        ...
        ttk.Button(buttons_frame, text="💾 Guardar", command=self.save_client, style="Accent.TButton").pack(side=tk.LEFT, padx=5)
        ttk.Button(buttons_frame, text="✏️ Editar", command=self.edit_client).pack(side=tk.LEFT, padx=5)
        ttk.Button(buttons_frame, text="🗑️ Eliminar", command=self.delete_client).pack(side=tk.LEFT, padx=5)
        ...
        filter_combo = ttk.Combobox(filter_frame, textvariable=self.filter_var, values=["todos", "activos", "inactivos"], state="readonly", width=10)
        ...
        ttk.Button(export_frame, text="📤 Exportar CSV", command=self.export_to_csv).pack(side=tk.LEFT, padx=5)
        ttk.Button(export_frame, text="📥 Importar CSV", command=self.import_from_csv).pack(side=tk.LEFT, padx=5)
        ttk.Button(export_frame, text="📊 Estadísticas", command=self.show_statistics).pack(side=tk.RIGHT, padx=5)
    ...
    def load_clients(self):
        ...
        for cliente in clientes:
            id_cliente, nombre, apellido, dni, telefono, email, direccion, fecha_registro, activo = cliente
            estado = "Activo" if activo else "Inactivo"
            
            # Insertar en el tree con tags según estado
            tag = "activo" if activo else "inactivo"
            self.tree.insert("", tk.END, values=(id_cliente, nombre, apellido, dni or "N/A", telefono or "N/A", email or "N/A", estado), tags=(tag,))
    ...
    def search_clients(self, *args):
        ...
        clientes = self.db.fetch(query, (search_pattern, search_pattern, search_pattern, search_pattern, search_pattern))
    ...
    def load_client_details(self, cliente_id):
        ...
        if cliente:
            ...
            id_cliente, nombre, apellido, dni, telefono, email, direccion, fecha_registro, activo = cliente_data
        ...
        try:
            if self.cliente_id_seleccionado:
                ...
                query = """
                    ...
                    WHERE id=?
                """
                self.db.execute(query, (nombre, apellido, dni, telefono, email, direccion, activo, self.cliente_id_seleccionado))
                ...
                query = """
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """
                self.db.execute(query, (nombre, apellido, dni, telefono, email, direccion, fecha_registro, activo))
            ...
            if "UNIQUE constraint failed" in str(e):
                messagebox.showerror("Error", "El DNI ya está registrado para otro cliente")
            else:
    ...
    def edit_client(self):
        ...
        messagebox.showinfo("Modo Edición", "Modifique los campos y presione 'Guardar' para confirmar los cambios")
    ...
    def delete_client(self):
        ...
        ventas = self.db.fetch("SELECT COUNT(*) FROM Ventas WHERE cliente_id = ?", (self.cliente_id_seleccionado,))
        ...
        if tiene_ventas:
            ...
            if respuesta:
                ...
                if messagebox.askyesno("Confirmación", "⚠️ ADVERTENCIA: Se eliminarán también todas las ventas asociadas.\n¿Continuar?"):
                    self.db.execute("DELETE FROM DetalleVenta WHERE venta_id IN (SELECT id FROM Ventas WHERE cliente_id = ?)", (self.cliente_id_seleccionado,))
                    self.db.execute("DELETE FROM Ventas WHERE cliente_id = ?", (self.cliente_id_seleccionado,))
                    self.db.execute("DELETE FROM Clientes WHERE id = ?", (self.cliente_id_seleccionado,))
                    messagebox.showinfo("Éxito", "Cliente y ventas asociadas eliminados")
        else:
            # Cliente sin ventas, eliminación simple
            if messagebox.askyesno("Confirmación", "¿Está seguro de eliminar este cliente?"):
                self.db.execute("DELETE FROM Clientes WHERE id = ?", (self.cliente_id_seleccionado,))
        ...
        if filename:
            try:
                ...
                with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
                    ...
                    # Encabezados
                    writer.writerow(["ID", "Nombre", "Apellido", "DNI", "Teléfono", "Email", "Dirección", "Fecha Registro", "Activo"])
    ...
    def show_statistics(self):
        ...
        try:
            ...
            clientes_activos = self.db.fetch("SELECT COUNT(*) FROM Clientes WHERE activo = 1")[0][0]
            clientes_inactivos = total_clientes - clientes_activos
            
            # Clientes con ventas
            clientes_con_ventas = self.db.fetch("SELECT COUNT(DISTINCT cliente_id) FROM Ventas WHERE cliente_id IS NOT NULL")[0][0]
            
            # Clientes registrados este mes
            fecha_inicio_mes = datetime.now().strftime("%Y-%m-01")
            clientes_este_mes = self.db.fetch("SELECT COUNT(*) FROM Clientes WHERE fecha_registro >= ?", (fecha_inicio_mes,))[0][0]
