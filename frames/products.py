...
class ProductFrame(ttk.Frame):
    ...
    def create_product_list(self):
        ...
        ttk.Label(
            self.list_frame, text="Listado de Productos", font=("Arial", 14, "bold")
        ).pack(pady=(0, 10))
        ...
        ttk.Button(btn_frame, text="Nuevo Producto", command=self.reset_form).pack(
            side="left", padx=5
        )

        ttk.Button(btn_frame, text="Eliminar", command=self.delete_product).pack(
            side="left", padx=5
        )
    ...
    def create_product_form(self):
        ...
        ttk.Label(
            self.form_frame, text="Formulario de Producto", font=("Arial", 14, "bold")
        ...
        self.desc_text = tk.Text(
            self.form_frame, height=4, width=25, font=("Arial", 10)
        )
        ...
        # Botón guardar
        ttk.Button(
            self.form_frame, text="Guardar Producto", command=self.save_product
    ...
    def save_product(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", "Precio, Stock y Proveedor ID deben ser números válidos."
            )
            return
        ...
        if self.product_id.get():
            ...
            query = """INSERT INTO Productos 
                       ...
                       VALUES (?, ?, ?, ?, ?)"""
            self.db.execute(
                query, (nombre, precio_val, stock_val, descripcion, prov_id_val)
            )
